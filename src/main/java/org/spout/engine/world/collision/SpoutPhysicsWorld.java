...
import com.bulletphysics.collision.shapes.voxel.VoxelCollisionInfo;
...
public class SpoutPhysicsWorld implements VoxelPhysicsWorld {
	...
	@Override
	public VoxelCollisionInfo getCollisionShapeAt(int x, int y, int z) {
		final SpoutBlock block = (SpoutBlock) simulation.getBlock(x, y, z, null);
		return new SpoutVoxelCollisionInfo(block.getMaterial(), block.getPosition());
	}

	private static class SpoutVoxelCollisionInfo implements VoxelCollisionInfo {
		private final boolean isColliding, isBlocking;
		private final CollisionShape shape;
		private final Vector3 position;

		public SpoutVoxelCollisionInfo(BlockMaterial material, Vector3 position) {
